...
The **Timer** plugin is a speedrun stopwatch. Press the split shortcut to start a run and again to record each lap, the finish shortcut to stop, and the reset shortcut (or **Reset**) to clear it. The shortcuts default to Ctrl+Alt+Shift+S, F, and R and can be changed on the **Shortcuts** tab in Settings, including to a single numpad, F13–F24, Pause, Scroll Lock, or Insert key. A key bound on its own stops reaching games and other apps while FourFold is open. Single-key shortcuts only fire when no Ctrl, Alt, or Shift key is held, so avoid holding a modifier while splitting. In full screen, switch the Timer card on in the Overlays panel and drag it anywhere. Runs are not saved when the app closes.
