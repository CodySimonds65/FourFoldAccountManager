...
5. The desktop uses the approved public HTTPS service root embedded in its source by default, so released executables connect without extra setup. Set `FOURFOLD_LEADERBOARD_URL` when launching a development or test build to override that address. The URL is public, not secret. The desktop rejects HTTP, credentials in the URL, paths, queries, and fragments.
